...
def register(host):
    ...
    # uses; rtmlib's bundled YOLOX ONNX is only used when there is none.
    def _persons():
        try:
    ...
    for pid, label, kind, sizes, types, note in (
        ...
        host.provide_model(
            ...
            loader=(lambda k=kind: (lambda sz: (lambda img, *a, **kw: _pose_core.rtm_people(img, k, sz, _persons())))(
